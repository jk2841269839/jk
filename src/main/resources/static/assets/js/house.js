layui.define(['element', 'carousel', 'table', 'util'], function(exports){
    ...
    $("#house-login").find(".getCode").children("button").on('click', function() {
        // 获取手机号页面元素
        let mobileScope = $("#mobile");
        // 获取手机号
        let mobile = mobileScope.val();
        // 定义手机号正则表达式
        ...
        if (mobileRex.test(mobile)) {
            ...
            //倒计时总时间，为了演示效果，设为5秒，一般都是60s
            $.ajax({
                // 请求方式
                type: "post",
                // 请求地址
                url: "/phone.action?methodName=sendSms",
                // 允许ajax跨域携带cookie
                xhrFields: {
            ...
            let initCode = setInterval(() => {
                if (countdown != 0) {
                    $("#getCodeBtn").html("重新发送(" + countdown-- + ")");
                } else {
                    // 移除按钮禁用样式
    ...
    form.on('submit(user-login)', function(data) {
        ...
        $.ajax({
            ...
            type: "post",
            // 请求地址
            url: "/person.action?methodName=login",
            // 允许ajax跨域携带cookie
            xhrFields: {
                withCredentials: true
            },
            ...
            // 传值失败    data: data.field,
            // 返回值数据类型
            dataType: "json",
            success: function(response) {
                // 进不来，下两行无效
                window.console.log(response);
                location.replace("/jumpPage.action?methodName=toDefaultPage");
            },
            error: function() {
                window.console.log("服务器大大病了")
            }
        });
    });

    // 绑定测试
    $(".slider-content").children(".main-btn").on('click', function(){
        let value = $(".shangping").val();
